...
layui.define(['jquery', 'table'], function(exports) {
	...
	function getSugTable(opt) {
		...
		//下拉表格初始化table容器
		let html = '<table id="yutons_sug_' + opt.obj.getAttribute("id") + '" lay-filter="yutons_sug_' + opt.obj.getAttribute("id") +'"></table>';
		$("#" + opt.obj.getAttribute("id")).next().show().html(html);
        var eel = "yutons_sug_" + opt.id;
        var d1 = opt.obj.getAttribute("id");
		//下拉表格初始化
        ...
        table.on('rowDouble(' + eel + ')', function(obj) {
			//获取选中行所传入字段的值
            $("#" + opt.id).val(obj.data[opt.id]);
            $("#" + opt.id1).val(obj.data[opt.id1]);
            $("#" + opt.id2).val(obj.data[opt.id2]);
            $("#" + opt.id3).val(obj.data[opt.id3]);
            $("#" + opt.id4).val(obj.data[opt.id4]);
			$("#" + opt.id).next().hide().html("");
        });
